  ...
  module.controller('percentiles', function ($scope, querySrv, dashboard, filterSrv, $http) {
    ...
    $scope.get_data = function () {
      ...
      $.each(queries, function (i, q) {
        ...
        var qname = 'stats_'+i;
        var aggsquery = {};
        //query["must"]().push({"query_string": {"query":$scope.panel.add_query}})
        query['should']()[0]['query_string']['query'] = query['should']()[0]['query_string']['query'] + ' AND ' + $scope.panel.add_query
        aggsquery[qname] = {
          'percentiles': {
            'field': $scope.panel.field,
            'percents': percents
      ...
      results.then(function(results) {
        ...
        var value = results.data.aggregations.stats['doc_count'] //parseInt(results.data.aggregations.stats['stats'].values[$scope.panel.mode+".0"]);
